...
import React, { useEffect, useState } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { Edit, ChevronDown, MapPin, Calendar, Ruler, DollarSign, Package, Truck, Scale, Users, FileText, Download, Airplay, X, Loader2 } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { ModificationRequest } from './ModificationRequest';
import axios from 'axios';
...
const ShipmentCard = ({ req }) => {
    ...
    const { id, status, pickupAddressLine2, dropAddressLine2, pickupState, dropState, materialType, expectedPickupDate, expectedDeliveryDate, pickupAddressLine1, pickupPincode, dropAddressLine1, dropPincode, weightKg, lengthFt, customMaterialType, widthFt, heightFt, bodyType, truckSize, manpower, noOfLabours, materialValue, additionalNotes, ebayBillUrl, transportMode, coolingType, offerPrice } = shipmentData;
            ...
            <motion.div layout className="bg-white border border-black/10 shadow-sm rounded-xl">
                ...
                <AnimatePresence>
                    {expanded && (
                        ...
                        >
                            <div className="border-t border-black/10 p-4 space-y-6">
                                {/* Details Sections... (No changes needed here, they read from `shipmentData`) */}
                            <DetailSection title="Route">
                                <DetailItem icon={<MapPin size={14} />} label="Pickup Address" value={`${pickupAddressLine1}, ${pickupState} - ${pickupPincode}`} />
                                <DetailItem icon={<MapPin size={14} />} label="Drop Address" value={`${dropAddressLine1}, ${dropState} - ${dropPincode}`} />
                            </DetailSection>
                            ...
                            <DetailSection title="Logistics & Value">
                                ...
                                {status === 'REQUESTED' && (
                                    <div className="pt-4 flex justify-end gap-2">
                                        <Button variant="outline" size="sm" className="bg-red-500 hover:bg-red-600 text-white" onClick={handleReject}>
                                            <X size={14} className="mr-2" /> REJECT
                                        </Button>
                                        <Button variant="outline" size="sm" className="bg-green-500 hover:bg-green-600 text-white" onClick={() => setIsOffering(true)}>
                                            <Edit size={14} className="mr-2" /> OFFER PRICE
                                        </Button>
                                    </div>
                                )}
